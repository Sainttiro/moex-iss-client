...
### DAG `moex_day_session_load`

DAG `moex_day_session_load` настроен для ежедневного запуска в 19:00 по московскому времени в рабочие дни. Он выполняет скрипт `scripts/day_session_load.py`, который загружает данные по итогам дневной торговой сессии в отдельную базу данных ClickHouse.
